...
const handleChartClick = (params) => {
  console.log('气泡图点击:', props.title, params)
  ...
  if (clickedValue) {
    ...
    console.log('✅ 发送气泡图点击事件:', eventData)
    emit('chart-click', eventData)
  } else {
    console.log('❌ 无法获取气泡图点击值:', params)
  }
}
...
const exportChart = async (format = 'png') => {
  ...
  try {
    ...
    ElMessage.success(`气泡图已导出为${format.toUpperCase()}格式`)
  } catch (error) {
    console.error('导出气泡图失败:', error)
    ElMessage.error(`导出气泡图失败：${error.message || '请重试'}`)
  } finally {
...
const exportChartAsPDF = async () => {
  ...
  try {
    ...
    ElMessage.success('气泡图已导出为PDF格式')
  } catch (error) {
    console.error('导出气泡图PDF失败:', error)
    ElMessage.error(`导出气泡图PDF失败：${error.message || '请重试'}`)
  } finally {
    isExporting.value = false
  }
}
...
const exportFullscreenChart = async (format = 'png') => {
  ...
  try {
    ...
    ElMessage.success(`全屏气泡图已导出为${format.toUpperCase()}格式`)
  } catch (error) {
    console.error('导出全屏气泡图失败:', error)
    ElMessage.error(`导出全屏气泡图失败：${error.message || '请重试'}`)
  } finally {
    isExporting.value = false
  }
}
...
const exportFullscreenChartAsPDF = async () => {
  ...
  try {
    ...
    ElMessage.success('全屏气泡图已导出为PDF格式')
  } catch (error) {
    console.error('导出全屏气泡图PDF失败:', error)
    ElMessage.error(`导出全屏气泡图PDF失败：${error.message || '请重试'}`)
  } finally {
    isExporting.value = false
  }
}
...
const chartOption = computed(() => {
  ...
  if (!values || values.length === 0) {
    console.log('气泡图: 无数据可用')
    return {}
  }

  console.log('气泡图数据:', { labels, values })
