...
// 上传相关
const uploadRef = ref()
const uploadUrl = ref('/api/employee/status-info/import')  // 使用相对路径，让Vite代理处理
...
// 表格列配置 - 包含employee_info表的所有列，包括状态相关字段
const tableColumns = ref([
  ...
  { prop: 'status', label: '状态', width: 80, sortable: 'custom' },
  ...
  { prop: 'remarks3', label: '备注3', width: 200, showOverflowTooltip: true },
...
const fetchEmployeeData = async () => {
  ...
  try {
    ...
    const response = await employeeApi.getStatusInfo(params)
...
const handleDownloadTemplate = () => {
  try {
    ...
    link.setAttribute('download', `在职离职信息导入模板_${new Date().toISOString().split('T')[0]}.csv`)
...
const handleExport = async () => {
  try {
    ...
    console.log('在职离职信息导出参数:', params)
    const response = await employeeApi.exportStatusInfo(params)
    console.log('在职离职信息导出响应:', response)

    if (response.success) {
      // 创建Excel文件并下载
      const data = response.data
      console.log('在职离职信息导出数据:', data)
      const headers = [
        '序列', '区域', '部门', '岗位', '名字', '性别', '民族', '政治面貌', '员工性质', '险种',
        '出生日期', '生日', '入职时间', '实际转正日期', '备注', '合同终止日期', '工龄（月）',
        '身份证号', '身份证地址', '年龄', '籍贯', '毕业院校', '专业', '学历', '教育方式',
      ...
      const url = URL.createObjectURL(blob)
      link.setAttribute('href', url)
      link.setAttribute('download', `在职离职信息_${new Date().toISOString().split('T')[0]}.csv`)
      link.style.visibility = 'hidden'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
